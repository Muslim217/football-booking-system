...
struct FieldDetailView: View {
    ...
    private let weekDays = (0..<7).compactMap { Calendar.current.date(byAdding: .day, value: $0, to: Date()) }
                ...
                VStack(spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            HStack(spacing: 4) {
                                Image(systemName: "location.fill")
                                    .font(.system(size: 11)).foregroundColor(.fbTextFaint)
                                Text(field.address)
                        ...
                        if let desc = field.description, !desc.isEmpty {
                            Text(desc)
                                .font(.system(size: 14)).foregroundColor(.fbTextMuted).lineSpacing(3)
                        ...
                        HStack(spacing: 6) {
                            ForEach(weekDays, id: \.self) { date in
                                Button { selectedDate = date } label: {
                                    ...
                                    .padding(.vertical, 8)
                                    .background(Calendar.current.isDate(date, inSameDayAs: selectedDate) ? Color.fbPrimary : Color.fbSurface)
                                    .foregroundColor(Calendar.current.isDate(date, inSameDayAs: selectedDate) ? .white : .fbText)
                                    .overlay(RoundedRectangle(cornerRadius: 10)
                                        .strokeBorder(Calendar.current.isDate(date, inSameDayAs: selectedDate) ? Color.fbPrimary : Color.fbBorder, lineWidth: 1))
                                    .cornerRadius(10)
                                }
                                .buttonStyle(.plain)
                            }
                                ...
                                ForEach(busyOnDay) { booking in
                                    HStack(spacing: 8) {
                                        Image(systemName: "clock.fill")
                                            .font(.system(size: 10)).foregroundColor(.fbWarning)
