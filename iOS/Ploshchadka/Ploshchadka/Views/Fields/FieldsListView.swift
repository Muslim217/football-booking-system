...
struct FieldsListView: View {
    ...
    private var filtered: [Field] {
        fields.filter {
            (activeFilter.isEmpty || $0.fieldType == activeFilter) &&
            (searchText.isEmpty ||
...
// MARK: - Field Card (горизонтальный как в дизайне)
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(spacing: 4) {
                    Image(systemName: "location.fill")
                        .font(.system(size: 10)).foregroundColor(.fbTextFaint)
                    Text(field.address)
